...
class Linear_ReLU(object):
    ...
    def forward(x, w, b):
        ...
        # Replace "pass" statement with your code
        x_reshaped = x.view(x.shape[0], -1)
        out_f = torch.matmul(x_reshaped, w) + b
        out = torch.relu(out_f)
        cache = (x, w, b, out_f , out)
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
        return out, cache
    ...
    def backward(dout, cache):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        x, w, b , out_f, out_relu= cache 
        dout_relu = dout*(out_relu>0).float()
        dx = torch.matmul(dout_relu , w.t()).reshape(x.shape)
        dw = torch.matmul(x.reshape(x.shape[0] , -1).t() ,dout_relu) 
        db = torch.sum(dout_relu, dim=0)
        ######################################################################
        #                END OF YOUR CODE                                    #
        ######################################################################
        return dx, dw, db
...
def softmax_loss(x, y):
    ...
    # -1/N * sigma(log(exp(x(i, yi)/sigma(xj))))
    loss = -torch.log(probs[range(x.shape[0]), y]).mean()
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        # it is supposed dtype to be chosen when calling the class. (it can be changed to double if needed)
    ...
    def loss(self, X, y=None):
        ...
        # add reg term
        reg_term= self.reg * (torch.norm(self.params['W1'])**2 + torch.norm(self.params['W2'])**2)

        loss += reg_term

        grad_L2, dW2, db2 = Linear.backward(grad_out, cache_L2)
        grad_L1, dW1, db1 = Linear_ReLU.backward(grad_L2, cache_L1)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(1 , self.num_layers-1) : 
            self.params['W'+str(i+1)] = torch.normal(0, weight_scale, size=(hidden_dims[i-1], hidden_dims[i]), dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            if i == self.num_layers - 1:
                ...
                scores, cache = Linear.forward(X, self.params['W'+str(i)], self.params['b'+str(i)])
            else:
                # Hidden layers with ReLU and dropout
                X, cache_relu = Linear_ReLU.forward(X, self.params['W'+str(i)], self.params['b'+str(i)])
                if self.use_dropout:
                    X, cache_dropout = Dropout.forward(X, self.dropout_param)
                    cache = (cache_relu, cache_dropout)
                else:
                    cache = (cache_relu,)
            caches.append(cache)
        ...
        reg_term = 0
        for i in range(1, self.num_layers):
            reg_term += 0.5 * self.reg * torch.norm(self.params['W'+str(i)])**2
        loss += reg_term
    
        for i in range(self.num_layers-1, 0, -1):
            if i == self.num_layers - 1:
                # Last layer without ReLU...
                grad_out, grads['W'+str(i)], grads['b'+str(i)] = Linear.backward(grad_out, caches[i-1])
            else:
                # Hidden layers with ReLU and dropout...
                if self.use_dropout:
                    cache_relu, cache_dropout = caches[i-1]
                    grad_out = Dropout.backward(grad_out, cache_dropout)
                else:
                    cache_relu, = caches[i-1]
                grad_out, grads['W'+str(i)], grads['b'+str(i)] = Linear_ReLU.backward(grad_out, cache_relu)
...
def get_three_layer_network_params():
    ...
    ###############################################################
    # we are asked to changed these values
    learning_rate = 5.5e-2 # Experiment with this!
    weight_scale = 4e-1   # Experiment with this!
    ################################################################
    #                             END OF YOUR CODE                 #
    ################################################################
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        - dropout_param: A dictionary with the following keys:
          ...
          - seed: Seed for the random number generator. Passing seed
            makes this function deterministic, which is needed for
            gradient checking but not in real networks.
        Outputs:
        - out: Tensor of the same shape as x.
        - cache: tuple (dropout_param, mask). In training mode, mask
          is the dropout mask that was used to multiply the input;
    ...
    def backward(dout, cache):
        ...
        dx = None
        if mode == 'train':
            # During training, apply the dropout mask to the upstream gradients
            dx = dout * mask
        elif mode == 'test':
            # During testing, no dropout is applied, so the gradients are unchanged
            dx = dout
